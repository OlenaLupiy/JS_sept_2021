...
for (const user of usersList) {
    ...
    for (const addressKey in user.address) {
        ...
        streetDiv.innerText+= `${addressKey} - ${user.address[addressKey]}`

        addressDiv.append(streetDiv);
    }
    for (const geoKey in user.address.geo) {
        let geoElement = document.createElement('div');
        geoElement.innerText+= geoKey + user.address.geo[geoKey];
        geoDiv.append(geoElement);
    }
    for (const companyKey in user.company) {
        let elementCompany = document.createElement('div');
        elementCompany.innerText+= `${companyKey} - ${user.company[companyKey]}`;
        companyDiv.append(elementCompany)
    }

    divUser.appendChild(idDiv);
    divUser.appendChild(nameDiv);
    divUser.appendChild(usernameDiv);
    divUser.appendChild(emailDiv);
    divUser.appendChild(addressDiv);
    divUser.appendChild(phoneDiv)
    divUser.appendChild(websiteDiv);
